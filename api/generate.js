...
Your main goal is clarity and structure without changing my voice. This should feel like I’m speaking, just clearer and more readable.
...
export default async function handler(request, response) {
  ...
  try {
    ...
    Return ONLY a raw JSON object (no markdown, no code blocks) with this structure:
    {
      ...
      "subHeadline": "A short context line (max 7 words)"
    }`;

    if (action === 'generatePost') {
        const lengthText = length === 'Short' ? "Short & Punchy" : "Thoughtful Storytelling";
        userPrompt = `
        TASK: Write a personal LinkedIn post based on the INPUT below.
        STYLE: ${lengthText}
        INPUT: "${rawInput}"
    ...
    try {
        ...
        if (action === 'generatePost') {
            ...
            if (!jsonResponse.subHeadline) jsonResponse.subHeadline = "The description";
        }
